...
import { MAINTENANCE_STATUSES } from "@hospitalityos/shared";
...
export function MaintenancePage() {
  ...
  async function setStatus(roomId: string, status: string) {
    try {
      await apiSend(`/accommodation/rooms/${roomId}/maintenance`, "PATCH", { status });
    ...
    <section className="page">
      <header className="page-header">
        <div>
          ...
          <h2>Maintenance</h2>
          <p className="muted">
            Available, scheduled, under maintenance, blocked — blocked rooms cannot be reserved.
          </p>
        </div>
      </header>
        ...
        {rooms.map((room) => (
          <article key={room.id} className="panel soft row-between">
            <div>
              ...
              <p className="muted small">{room.notes ?? "No notes"}</p>
            </div>
            <select
